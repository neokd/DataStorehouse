...
def main():
    ...
    file_list = os.listdir(current_directory)  # List files in the current directory

    # Filter the list to include only CSV and JSON files
    ...
    if valid_files:
        ...
        # Enter the number assigned to the file
        selection = click.prompt("Enter the number of the file to analyze", type=int)

        if 1 <= selection <= len(valid_files):
